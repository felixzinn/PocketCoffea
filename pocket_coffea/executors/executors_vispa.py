...
class DaskExecutorFactory(ExecutorFactoryABC):
    ...
    def setup(self):
        ...
        self.cluster = HTCondorCluster(
            cores=1,
            processes=1,
            memory=self.run_options.get("mem-per-worker", "4000MiB"),
            disk="2GB",
            local_directory="/tmp",
            log_directory=os.path.join(
                os.path.abspath("/tmp/FelixZinn/dask_logs"),
        ...
        # self.cluster.adapt(
        #     minimum_jobs=1, maximum_jobs=self.run_options.get("scaleout", 10)
        # )
        self.cluster.scale(self.run_options.get("scaleout", 10))
    ...
    def get(self):
        ...
        return DaskExecutor(
            client=self.client, treereduction=10, retries=0, compression=1
        )
